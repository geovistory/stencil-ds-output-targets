...
type CProps<ElementType> = Omit<
  StencilReactInternalProps<ElementType>,
  'children' | 'forwardedRef' | 'style' | 'className' | 'ref'
>;
export type NewProps<ElementType> = Omit<
  StencilReactInternalProps<ElementType>,
  'forwardedRef'
>;
...
>(
  ...
  const ReactComponent = (props: StencilReactInternalProps<ElementType>) => {
    ...
    const [sse, error] = useSSE(async () => {
      ...
      const { serverRenderWebComponent } = await import(
        './utils/serverRenderWebComponent'
      );
